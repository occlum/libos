...
submodule: githooks
	...
	cd deps/no-std-net && git apply ../no-std-net.patch >/dev/null 2>&1 || git apply ../no-std-net.patch -R --check
	cd deps/quick-error && git apply ../quick-error.patch >/dev/null 2>&1 || git apply ../quick-error.patch -R --check
